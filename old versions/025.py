...
def main():
    ...
    if date_input:
        try:
            if '-' in date_input:
                ...
                single_date = datetime.datetime.strptime(date_input, "%d/%m/%Y").date()
                start_date = single_date
                end_date = single_date
            use_date_filter = True
        except Exception as e:
            print(f"Date input error: {e}. Continuing without date filtering.")
            use_date_filter = False
    ...
    if use_date_filter:
        print(f"Found {len(files_to_process)} files to process based on the date filter.")
    else:
