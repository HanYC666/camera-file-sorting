...
class App(tk.Tk):
    ...
    def show_config_editor(self):
        ...
        for i, item in enumerate(self.source_data, start=1):
            source_text = f"{item['camera']} - {item['datetime']}"
            ...
            self.destination_paths[i] = {"label": dest_label, "files": [item]}
    ...
    def run_process(self):
        ...
        popup.transient(self)
        popup.grab_set()
        tk.Label(popup, text="Transferring Files...", font=("Arial", 12)).pack(pady=30)

        # Copy files to respective destination folders
        ...
        # Close the "Transferring Files" popup
        popup.destroy()
        messagebox.showinfo("Success", "Files have been successfully transferred!")
