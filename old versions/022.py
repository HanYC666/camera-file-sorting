...
def secure_copy(source_file_path, target_file_path):
    ...
    It copies the file preserving metadata and then verifies the file sizes.
    """
    try:
        ...
        if source_size != target_size:
            print(f"Warning: Size mismatch after copying {os.path.basename(source_file_path)}")
        else:
            print(f"Successfully copied {os.path.basename(source_file_path)} to {os.path.dirname(target_file_path)}")
    except Exception as e:
...
def main():
    ...
    # and collect camera models from EXIF metadata.
    ...
    # Build list of files to process (recursively)
    files_to_process = []
    for root, dirs, files in os.walk(source_folder):
        for file_name in files:
            if file_name.startswith('.') or file_name.startswith('_'):
    ...
    for root, file_name in files_to_process:
        ...
        if ext in photo_extensions:
            ...
            print(f"Copying photo {file_name} to {target_folder}")
            secure_copy(source_file_path, target_file_path)
        # Process video files
            ...
            create_directory(target_folder)
            target_file_path = os.path.join(target_folder, file_name)
            print(f"Copying video {file_name} to {target_folder}")
            secure_copy(source_file_path, target_file_path)
        else:
