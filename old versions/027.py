...
def main():
    ...
    date_input = input("Enter a date (DD/MM/YYYY) or a range (DD/MM/YYYY - DD/MM/YYYY), or leave empty: ").strip()
    ...
    if not camera_models_found:
        chosen_camera_model = input("No camera model found. Enter camera model for tagging videos: ").strip()
    elif len(camera_models_found) == 1:
        chosen_camera_model = list(camera_models_found)[0]
        print(f"Using detected camera model: {chosen_camera_model}.")
    else:
        ...
        while True:
            try:
                choice = int(input("Select a number for the camera model: ").strip())
