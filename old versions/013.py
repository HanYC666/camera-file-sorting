...
class App(tk.Tk):
    def __init__(self):
        ...
        self.source_data = []  # Store metadata (camera model and date shot)
        self.show_home()
    ...
    def show_home(self):
        ...
        tk.Label(self.main_frame, text="Home", font=("Arial", 24, "bold"), bg="white").pack(pady=20)
    ...
    def process_metadata(self, folder):
        ...
        # Show the "Reading Metadata" window
        popup = tk.Toplevel(self)
        popup.title("Processing")
        popup.geometry("300x100")
        popup.transient(self)
        popup.grab_set()
        tk.Label(popup, text="Reading Metadata...", font=("Arial", 12)).pack(pady=30)
                    ...
                    try:
                        ...
                        for line in output_lines:
                            ...
                            if "Date/Time Original" in line:
                                date_shot = line.split(":")[1].strip().split(" ")[0]  # Extract only the date

                        if camera_model and date_shot:
                            self.source_data.append({"camera": camera_model, "date": date_shot})

                    except Exception as e:
                        print(f"Error reading metadata from {file}: {e}")

        # Close the "Reading Metadata" popup
        popup.destroy()

        # Automatically open the configuration editor with the populated data
    ...
    def show_config_editor(self):
        ...
        for i, item in enumerate(self.source_data, start=1):
            source_text = f"{item['camera']} - {item['date']}"

            # Source Node
            tk.Label(content_frame, text=source_text, bg="white", font=("Arial", 10), anchor="w").grid(row=i, column=0, padx=10, pady=5, sticky="w")
            ...
            self.destination_paths[i] = dest_label
    ...
    def select_destination(self, idx):
        ...
        folder = filedialog.askdirectory()
        if folder:
            self.destination_paths[idx].config(text=folder)

    def run_process(self):
        """Run the configuration editor processes."""
        messagebox.showinfo("Info", "RUN clicked! Processing files...")
